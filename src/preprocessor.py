...
def vectorize_and_filter_tokens(column_data):
    """
    Transforms raw text into a TF-IDF matrix while filtering tokens based on frequency.
    Returns the sparse matrix and a 'reconstructed' version of the text containing
    only the tokens that survived the filtering process (e.g., removal of corpus-wide
    common words or rare terms).
    """
    documents = column_data.tolist()
    
    vectorizer = TfidfVectorizer(
        max_df=0.8, # Ignore terms that appear in more than x% of the documents
        min_df=2, # Ignore terms that appear in fewer than x documents
        ngram_range=(1, 2) # Create n-grams from 1 to 2 tokens
    )
    ...
    # Transform: "fire truck" -> "fire_truck"
    # Then join tokens back into a single document string
    filtered_docs = []
    for tokens in surviving_tokens_per_doc:
        # Replace internal spaces with underscores for each token for readability
...
def read_dataset(use_bert=False):
    ...
    X_tfidf, df['token_text'] = vectorize_and_filter_tokens(df['spacy_processed_text'])
